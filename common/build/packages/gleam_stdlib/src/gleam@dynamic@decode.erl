...
-opaque decoder(BVQ) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BVQ, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BVY)) -> {ok, BVY} |
...
-spec map(decoder(BZW), fun((BZW) -> BZY)) -> decoder(BZY).
...
-spec then(decoder(CAI), fun((CAI) -> decoder(CAK))) -> decoder(CAK).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CAN), list(decoder(CAN))) -> decoder(CAN).
...
-file("src/gleam/dynamic/decode.gleam", 1076).
?DOC(
...
-spec recursive(fun(() -> decoder(CBD))) -> decoder(CBD).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(BZS)) -> decoder(gleam@option:option(BZS)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BXT} | {error, BXT})
) -> {BXT, list(decode_error())}.
...
-spec collapse_errors(decoder(CAF), binary()) -> decoder(CAF).
...
-file("src/gleam/dynamic/decode.gleam", 998).
?DOC(
...
-spec failure(CAX, binary()) -> decoder(CAX).
failure(Placeholder, Name) ->
    {decoder, fun(D) -> {Placeholder, decode_error(Name, D)} end}.

-file("src/gleam/dynamic/decode.gleam", 1039).
?DOC(
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CAZ} | {error, CAZ})
) -> decoder(CAZ).
...
-spec fold_dict(
    {gleam@dict:dict(BZE, BZF), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {BZE, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {BZF, list(decode_error())})
) -> {gleam@dict:dict(BZE, BZF), list(decode_error())}.
...
-spec dict(decoder(BYX), decoder(BYZ)) -> decoder(gleam@dict:dict(BYX, BYZ)).
dict(Key, Value) ->
...
-spec list(decoder(BYL)) -> decoder(list(BYL)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BWU, list(decode_error())}, list(any())) -> {BWU,
    list(decode_error())}.
...
-spec index(
    list(BWI),
    list(BWI),
    fun((gleam@dynamic:dynamic_()) -> {BWL, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BWI)) -> {BWL, list(decode_error())})
) -> {BWL, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BVT), fun((BVT) -> decoder(BVV))) -> decoder(BVV).
...
-spec at(list(any()), decoder(BWF)) -> decoder(BWF).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BXD), fun((BXD) -> decoder(BXF))) -> decoder(BXF).
...
-spec optional_field(any(), BXJ, decoder(BXJ), fun((BXJ) -> decoder(BXL))) -> decoder(BXL).
...
-file("src/gleam/dynamic/decode.gleam", 597).
?DOC(
    " A decoder that decodes a value that is nested within other values. For\n"
    " example, decoding a value that is within some deeply nested JSON objects.\n"
    "\n"
...
-spec optionally_at(list(any()), BXQ, decoder(BXQ)) -> decoder(BXQ).
