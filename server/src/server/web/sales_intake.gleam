...
pub type ProductInput {
  ProductInput(nome: String, ambiente: String, quantidade: Int)
}
...
fn insert_products(
  ...
  case products {
    ...
    [p, ..rest] -> {
      let #(sql2, params2) =
        sql.add_sales_intake_product(sale_id, p.nome, p.ambiente, p.quantidade)
  ...
  case decode.run(json, register_decoder()) {
    ...
    Ok(RegisterInput(username, supplier, products)) -> {
        ...
        })
      {
        Ok(id) ->
          wisp.json_response(
            json.to_string(json.object([#("id", json.int(id))])),
            201,
          )
